function h_optim = curve_fit_2d(planar_arm_obj, g_tags_meas, t_tags, muscle_ids, init_val, offsets)
...
arguments
    ...
    muscle_ids
    init_val = [1; 0; -0.5];
    offsets = repelem({eye(3)}, length(g_tags_meas));
end
...
for i = 1 : tag_num
    ...
    % Right subtraction
    g_o_Xi = inv(planar_arm_obj.muscle_o.g_0) * planar_arm_obj.muscles(muscle_ids(i)).g_0 * offsets{i};
...
h_optim = fminsearch(f_cost_total, init_val);
